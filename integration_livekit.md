...
### 5.1 Using sqlc for queries (keep migrations with golang-migrate)

Purpose: use golang-migrate for schema migrations (DDL) and sqlc to generate type-safe Go code for SQL queries (DML). sqlc does NOT manage migrations — it only reads SQL schema/queries and generates Go structs and methods.

Why switch:
- Remove hand-written SQL/inline DDL from Go sources (eg. `db/sql.db.go`).
- Single source of truth for SQL: .sql migration files + queries/*.sql for DML.
- Compile-time checked queries with generated Go types.

Quick steps to migrate to sqlc:

1) Extract current schema into a first migration

- Dump current schema (psql --schema-only) or copy from `db/sql.db.go` DDL.
- Create initial migration: `migrations/000001_init.up.sql` (all CREATE TABLE/INDEX/ENUM statements) and `000001_init.down.sql` to DROP in reverse order.
- Run `make migrate-up` against local DB to apply.

2) Add sqlc configuration (sqlc.yaml)
...
Notes:
- `schema` can point to the `migrations` directory (sql files). sqlc will parse CREATE TABLE statements to generate models.
- `queries` is where DML lives (SELECT/INSERT/UPDATE/DELETE). Keep these small and focused per entity.

3) Write queries as .sql files (examples)
...
```sql
-- name: CreateRoom :one
INSERT INTO rooms (id, host_id, name, status, created_at)
VALUES ($1, $2, $3, $4, NOW())
RETURNING id, host_id, name, status, created_at;
```

4) Generate code

- Install sqlc (`brew install sqlc` or `go install github.com/kyleconroy/sqlc/cmd/sqlc@latest`).
- Run: `sqlc generate` — this parses schema + queries and writes `internal/db` package with typed structs and query methods.

5) Replace inline DB code with generated package

- Remove or archive `db/sql.db.go` after migrating DDL to migrations.
- Use generated package: `q := db.New(conn)` then `q.CreateUser(ctx, params)` or `q.GetUserByID(ctx, id)`.
- sqlc returns Go structs matching columns; keep queries' SELECT lists stable to avoid breaking generated types.

6) Best practices & comments for understanding

- Keep migrations for DDL only. Use golang-migrate for versioned schema changes.
- Keep sqlc queries as the canonical DML. Naming convention: `-- name: <FuncName> :<one|many|exec|rows>`.
- Avoid `SELECT *` in queries. List columns explicitly to keep generated structs stable when adding columns.
- For complex transactions, write a `WITH` or transaction wrapper in Go using `conn.BeginTx` and call multiple sqlc methods within the same transaction.
- Add comments above queries explaining intent and which Go type is returned — helpful for future maintainers.

7) Example directory layout

```
/migrations/          # DDL migration files (versioned)
/queries/
  users.sql
  rooms.sql
  recordings.sql
...
internal/              # hand-written app code uses internal/db
```

8) Migration checklist when changing schema

- Create new migration with `migrate create -ext sql -dir migrations -seq add_column_xyz`
- Apply to a staging DB with `make migrate-up`
- Update queries/*.sql affected by schema change (add columns to SELECTs)
- Run `sqlc generate` and fix compile errors
- Run tests
- Deploy migration to production (run migrate-up) then deploy code.

9) Additional tips

- Use `sqlc`'s `overrides` in sqlc.yaml if using custom types (UUID, timestamptz) to map them to Go types.
- Keep generated package import path stable to avoid import churn.
- Document how to regenerate code in CONTRIBUTING.md: `make migrate-up && sqlc generate && go test ./...`.

Comments added inline here to help developers understand the split of responsibilities: migrations (golang-migrate) = schema history; sqlc = type-safe query generation. Both work together — migrations change the DB, sqlc keeps Go types in sync with the schema.
